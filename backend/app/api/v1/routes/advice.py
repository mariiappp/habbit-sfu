...
async def get_daily_advice(
    ...
    service: AdviceServiceDep,
    date: date_type | None = None,
) -> DailyAdviceResponse:
    """Return daily advice for the authenticated user."""
    target_date = date or date_type.today()
    try:
        payload = await service.get_daily_advice(
            token=token,
            target_date=target_date,
        )
        return DailyAdviceResponse(**payload)
